...
<body>
	...
	<div class="container">
		...
		<p class="page-subtitle">Track your past purchases and leave
			reviews</p>
					...
					<tbody>
						...
						try {
							...
							if (custRs.next()) {
								...
								PreparedStatement orderStmt = conn.prepareStatement("SELECT s.*, p.Name, p.Category, p.ProductID, "
								+ "(SELECT COUNT(*) FROM feedback f WHERE f.ProductID = p.ProductID AND f.CustomerID = ?) as has_feedback "
								+ "FROM sales s " + "JOIN products p ON s.ProductID = p.ProductID " + "WHERE s.CustomerID = ? "
 ...
 %> <a
								href="productpage.jsp?productId=<%=orderRs.getInt("ProductID")%>"
								class="btn review-btn"> <i class="fas fa-star me-1"></i>
									Rate & Review
							</a> <%
 }
 %>
						...
						<tr>
							<td colspan="8">
								<div class="empty-state">
									...
									<a href="homepage.jsp" class="btn browse-btn"> Browse
										Products </a>
								</div>
							</td>
						</tr>
